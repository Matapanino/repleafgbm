...
def build_grid() -> list[tuple[str, dict]]:
    ...
    return [
        ...
        # Phase 13: learned encoders (pretrained on the initial Newton
        # residual, then frozen). torch_periodic shares the frozen periodic's
        # initialization, so the frozen-vs-learned delta isolates training.
        ("torch_periodic k=4 + linear", {**emb, "encoder": "torch_periodic",
                                         "encoder_params": {"n_frequencies": 4}}),
        ("torch_plr 4bins x 4out", {**emb, "encoder": "torch_plr"}),
    ]
