...
representation $\zt(x)$---between classic GBDTs, whose leaves emit a constant,
and tabular deep learning, which embeds features but discards the axis-aligned
tree. Expressivity is added only inside the leaf, as an $h$-weighted ridge fit
of an affine model on $\zt(x)$. We give the additive/Newton formulation, the
leaf models, and a supervised \emph{pretrain-then-freeze} encoder, with a
proposition showing that updating the encoder during boosting breaks stage-wise
additivity---which is why it is frozen. Empirically we report an honest
picture. Under a shared hyperparameter-optimization budget on the Grinsztajn
benchmarks (ten seeds, significance-tested), RepLeafGBM is \emph{mid-pack}: a
clean third of five on both regression suites, fourth--fifth on classification.
Its distinctive value is in niche regimes: refitting an external GBDT's own
routes with embedded-linear leaves improves regression RMSE by $5$--$9\%$ on
synthetic targets and significantly (Wilcoxon $p=0.002$, no losses) on every
real regression dataset tested, and vector-leaf routing with robust objectives
is decisive under target contamination ($10/10$ seeds at every contamination
level on all five real multi-target datasets)---where we identify and fix a
scale bug in saturated-gradient (Huber/quantile) objectives via per-output
target standardization. Equally, we document where the idea does \emph{not}
help: binary classification (the logistic Hessian starves the leaf-linear fit)
and learned encoders, which win on structured synthetic data but do not
transfer to real targets. We release an open-source implementation with
parity-tested NumPy, Rust ($\sim$5.8$\times$), and CUDA backends.
\end{abstract}
...
$\times$ $10$ seeds), a $70/15/15$ split, and a 10k-row training cap; there is no
...
The honest summary (Table~\ref{tab:openml}) is that RepLeafGBM is a \emph{mid-pack}
learner with a clear task-type split. Its mean rank spans $3.00$--$3.81$ out of five: on
\emph{both regression suites} it is a clean third ($3.00$), ahead of XGBoost and HistGB,
while on the classification suites it slips to fourth--fifth ($3.81$/$3.71$)---consistent
with the binary-Hessian limitation documented in \S\ref{sec:exp-ablations} (the logistic
$h=p(1-p)$ starves the embedded-linear leaf, so the leaf-expressivity edge is a
regression-side phenomenon). It is never the leader, and it is never both significantly
\emph{and} practically worse than the best baseline (on numerical regression the Wilcoxon
test against the best baseline, CatBoost, is $p=0.37$; on numerical classification the
sign of a deficit against LightGBM is significant, $p=3.4\times10^{-3}$, but below the
minimum-relevant-difference band on most datasets). With ten seeds the categorical
regression suite is no longer statistically flat (Friedman $p=0.025$); only
categorical classification remains flat ($p=0.438$; seven datasets, correspondingly
low power). RepLeafGBM is thus competitive but not state-of-the-art on average---a
useful, honestly-positioned tabular learner whose distinctive value lies in regression
and in the niche regimes studied below, not in the aggregate leaderboard. Full
per-dataset tables and the critical-difference diagrams (Figure~\ref{fig:cd}) are
produced by the shipped leaderboard harness (Appendix~\ref{app:repro}).
...
\caption{Critical-difference diagrams (Nemenyi, $\alpha{=}0.05$, ten seeds) for the four
Grinsztajn suites under the shared HPO budget. Models joined by a bar are not
statistically separable. RepLeafGBM is a clean third on both regression suites and
fourth--fifth on classification (where it separates from the leading GBDT on numerical
classification); categorical classification is statistically flat.}
\label{fig:cd}
\end{figure}
...
among five tuned model families, $10$ seeds). RepLeafGBM is a clean third on both
regression suites and fourth--fifth on classification, with no significant-and-practical
loss against the best baseline anywhere. The Friedman $p$ is over all five models.}
\label{tab:openml}
\begin{tabular}{lcc}
\toprule
Grinsztajn suite (\# datasets) & RepLeafGBM mean rank (of $5$) & Friedman $p$ \\
\midrule
Numerical regression (19)      & $3.00$ & $2.2\times10^{-3}$ \\
Numerical classification (16)  & $3.81$ & $4.3\times10^{-4}$ \\
Categorical regression (17)    & $3.00$ & $0.025$ \\
...
learning is argued rather than measured. (vi) The fair leaderboard and the two niche
studies (robust multi-output, router extraction) all use ten seeds; the ten-seed
leaderboard replication sharpened the picture---a clean third on both regression suites
and a small, sign-significant (but mostly below the relevance band) deficit on numerical
classification---while the smaller synthetic and guard studies remain five-seed and
should be read as indicative. (vii) The per-output
...
classification), over ten seeds, with datasets capped at $20{,}000$ rows. All models
...
numpy 1.26.4, scikit-learn 1.9.0, lightgbm 4.6.0, xgboost 3.2.0, catboost 1.2.10,
