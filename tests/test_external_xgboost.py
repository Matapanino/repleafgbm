"""Tests for the XGBoost external_model backend (xgboost optional)."""
...
# Importing the package must work without xgboost; only usage requires it.
...
def _fast_base(task="regression"):
    return XGBoostExternalModel(
        task=task, random_state=0, n_estimators=30, max_depth=3
    )
...
def test_early_stopping_limits_predictions(regression_data):
    Xtr, ytr, Xte, yte = regression_data
    base = XGBoostExternalModel(
        task="regression", random_state=0, n_estimators=300, max_depth=3
    )
    ...
    # Leaf-index features must come from the best model only.
    leaves = base.predict_leaf_indices(Xte)
...
def test_unfitted_and_bad_task():
    ...
    with pytest.raises(ValueError, match="task"):
        XGBoostExternalModel(task="multiclass")
...
def test_missing_xgboost_message(monkeypatch, regression_data):
    """Without xgboost the error must say how to install it."""
    import sys

    Xtr, ytr, _, _ = regression_data
    monkeypatch.setitem(sys.modules, "xgboost", None)
    with pytest.raises(ImportError, match="pip install"):
        XGBoostExternalModel().fit(Xtr, ytr)
